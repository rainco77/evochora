...
public class Organism {
    ...
    Organism(int id, int[] startIp, int initialEnergy) {
        ...
        this.lastIp = Arrays.copyOf(startIp, startIp.length);
    }
    ...
    public Action planTick(World world, Map<Integer, BiFunction<Organism, World, Action>> plannerRegistry) {
        ...
        this.lastIp = Arrays.copyOf(this.ip, this.ip.length);
    ...
    public void executeAction(Action action, Simulation simulation) {
        ...
        if (!this.skipIpAdvance) {
            // KORRIGIERT: Hole das Opcode-Objekt und dann dessen Länge
    ...
    int fetchArgument(World world) {
        this.ip = getNextInstructionPosition(this.ip, world);
        return world.getSymbol(this.ip).value();
    }
    ...
    int[] getLastIp() { return this.lastIp; }

    // --- Öffentliche Getter ---
    public int getId() { return id; }
    public int[] getIp() { return Arrays.copyOf(ip, ip.length); }
    public int[] getIpBeforeFetch() { return Arrays.copyOf(lastIp, lastIp.length); }
