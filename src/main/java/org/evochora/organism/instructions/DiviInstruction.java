...
public class DiviInstruction extends Instruction {
    ...
    public DiviInstruction(Organism o, int r1, int literal, int fullOpcodeId) {
    ...
    public void execute(Simulation simulation) {
        ...
        if (val1Obj instanceof int[]) {
            organism.instructionFailed("DIVI: Vektoren können nicht dividiert werden.");
            return;
        }
        if (!(val1Obj instanceof Integer v1Raw)) {
            organism.instructionFailed("DIVI: Ungültiger Registertyp. Register " + reg1 + " muss ein skalarer Integer sein.");
            return;
        }
        Symbol s1 = Symbol.fromInt(v1Raw);
        Symbol s2 = Symbol.fromInt(this.literalValue);
        if (s2.toScalarValue() == 0) {
            organism.instructionFailed("DIVI: Division durch Null.");
            return;
        }
        if (Config.STRICT_TYPING && s1.type() != s2.type()) {
            organism.instructionFailed("DIVI: Register- und Literal-Typen müssen im strikten Modus übereinstimmen.");
            return;
        }
    ...
    public static Instruction plan(Organism organism, World world) {
        ...
        return new DiviInstruction(organism, r1, literal, fullOpcodeId);
    }

    public static AssemblerOutput assemble(String[] args, Map<String, Integer> registerMap, Map<String, Integer> labelMap) {
        if (args.length != 2) throw new IllegalArgumentException("DIVI erwartet genau 2 Argumente: %REG TYPE:WERT");
