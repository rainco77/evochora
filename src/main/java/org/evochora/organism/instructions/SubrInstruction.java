...
public class SubrInstruction extends Instruction {
    ...
    public SubrInstruction(Organism o, int r1, int r2, int fullOpcodeId) {
    ...
    public void execute(Simulation simulation) {
        ...
        if (val1Obj instanceof int[] v1 && val2Obj instanceof int[] v2) {
            if (v1.length != v2.length) {
                organism.instructionFailed("SUBR: Vektor-Dimensionen stimmen nicht überein.");
                return;
            }
            ...
            if (Config.STRICT_TYPING && s1.type() != s2.type()) {
                organism.instructionFailed("SUBR: Registertypen müssen im strikten Modus übereinstimmen.");
                return;
            }
    ...
    public static Instruction plan(Organism organism, World world) {
        ...
        return new SubrInstruction(organism, r1, r2, fullOpcodeId);
    }

    public static AssemblerOutput assemble(String[] args, Map<String, Integer> registerMap, Map<String, Integer> labelMap) {
        if (args.length != 2) throw new IllegalArgumentException("SUBR erwartet 2 Argumente: %REG1 %REG2");
