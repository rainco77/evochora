...
public class ShliInstruction extends Instruction {
    ...
    public ShliInstruction(Organism o, int r1, int literal, int fullOpcodeId) {
    ...
    public void execute(Simulation simulation) {
        ...
        if (val1Obj instanceof int[]) {
            organism.instructionFailed("SHLI: Vektoren sind nicht für bitweise Operationen erlaubt.");
            return;
        }
        if (!(val1Obj instanceof Integer v1Raw)) {
            organism.instructionFailed("SHLI: Ungültiger Registertyp. Register " + reg1 + " muss ein skalarer Integer sein.");
            return;
        }
        Symbol s1 = Symbol.fromInt(v1Raw);
        Symbol s2 = Symbol.fromInt(this.literalValue);
        if (Config.STRICT_TYPING && s1.type() != Config.TYPE_DATA) {
            organism.instructionFailed("SHLI: Register muss vom Typ DATA sein im strikten Modus.");
            return;
        }
        if (s2.type() != Config.TYPE_DATA) {
            organism.instructionFailed("SHLI: Literal muss vom Typ DATA sein.");
            return;
        }
    ...
    public static Instruction plan(Organism organism, World world) {
        ...
        return new ShliInstruction(organism, r1, literal, fullOpcodeId);
    }

    public static AssemblerOutput assemble(String[] args, Map<String, Integer> registerMap, Map<String, Integer> labelMap) {
        if (args.length != 2) throw new IllegalArgumentException("SHLI erwartet genau 2 Argumente: %REG DATA:WERT");
        ...
        if (literalParts.length != 2 || !literalParts[0].equalsIgnoreCase("DATA")) {
            throw new IllegalArgumentException("Literal für SHLI muss das Format DATA:WERT haben: " + args[1]);
        }
