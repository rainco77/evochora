...
public class IftiInstruction extends Instruction {
    ...
    public IftiInstruction(Organism o, int r1, int literal, int fullOpcodeId) {
    ...
    public void execute(Simulation simulation) {
        ...
        if (val1Obj instanceof Integer v1Raw) {
            int registerType = Symbol.fromInt(v1Raw).type();
            int literalType = Symbol.fromInt(this.literalValue).type();
            conditionMet = (registerType == literalType);
        }
        if (!conditionMet) {
    ...
    public static Instruction plan(Organism organism, World world) {
        ...
        return new IftiInstruction(organism, r1, literal, fullOpcodeId);
    }

    public static AssemblerOutput assemble(String[] args, Map<String, Integer> registerMap, Map<String, Integer> labelMap) {
        if (args.length != 2) throw new IllegalArgumentException("IFTI erwartet genau 2 Argumente: %REG TYPE:WERT");
