...
public class ShlsSInstruction extends Instruction {

    public static final int LENGTH = 1;

    public ShlsSInstruction(Organism organism, int fullOpcodeId) {
    ...
    public void execute(Simulation simulation) {
        ...
        try {
            ...
            if (count instanceof Integer ci && value instanceof Integer vi) {
                ...
                organism.instructionFailed("SHLS: Nur Skalar-Skalar unterstützt (value, count).");
            }
        } catch (NoSuchElementException e) {
            organism.instructionFailed("Stack Underflow: SHLS benötigt 2 Operanden.");
        }
    }

    public static Instruction plan(Organism organism, World world) {
        int full = world.getSymbol(organism.getIp()).toInt();
        return new ShlsSInstruction(organism, full);
    }

    public static AssemblerOutput assemble(String[] args, Map<String, Integer> regMap, Map<String, Integer> lblMap) {
        if (args.length != 0) throw new IllegalArgumentException("SHLS erwartet keine Argumente.");
