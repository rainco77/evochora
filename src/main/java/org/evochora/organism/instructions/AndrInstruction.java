...
public class AndrInstruction extends Instruction {
    ...
    public AndrInstruction(Organism o, int r1, int r2, int fullOpcodeId) {
    ...
    public void execute(Simulation simulation) {
        ...
        if (val1Obj instanceof int[] || val2Obj instanceof int[]) {
            organism.instructionFailed("ANDR: Vektoren sind nicht für bitweise Operationen erlaubt.");
            return;
        }
        if (!(val1Obj instanceof Integer v1Raw) || !(val2Obj instanceof Integer v2Raw)) {
            organism.instructionFailed("ANDR: Ungültige Registertypen. Beide Register müssen skalare Integer sein.");
            return;
        }
        Symbol s1 = Symbol.fromInt(v1Raw);
        Symbol s2 = Symbol.fromInt(v2Raw);
        if (Config.STRICT_TYPING && s1.type() != s2.type()) {
            organism.instructionFailed("ANDR: Registertypen müssen im strikten Modus übereinstimmen.");
            return;
        }
    ...
    public static Instruction plan(Organism organism, World world) {
        ...
        return new AndrInstruction(organism, r1, r2, fullOpcodeId);
    }

    public static AssemblerOutput assemble(String[] args, Map<String, Integer> registerMap, Map<String, Integer> labelMap) {
        if (args.length != 2) throw new IllegalArgumentException("ANDR erwartet 2 Argumente: %REG1 %REG2");
