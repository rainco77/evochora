...
public class GtsSInstruction extends Instruction {

    public static final int LENGTH = 1;

    public GtsSInstruction(Organism organism, int fullOpcodeId) {
    ...
    public void execute(Simulation simulation) {
        ...
        try {
            ...
            organism.instructionFailed("Stack Underflow: GTS benötigt 2 Operanden.");
        }
    }

    public static Instruction plan(Organism organism, World world) {
        int full = world.getSymbol(organism.getIp()).toInt();
        return new GtsSInstruction(organism, full);
    }

    public static AssemblerOutput assemble(String[] args, Map<String, Integer> registerMap, Map<String, Integer> labelMap) {
        if (args.length != 0) throw new IllegalArgumentException("GTS erwartet keine Argumente.");
