// src/main/java/org/evochora/organism/JmpInstruction.java
...
public class JmpInstruction extends Instruction {
    ...
    public JmpInstruction(Organism o, int r) {
        super(o);
        this.reg = r;
    }
    ...
    public ArgumentType getArgumentType(int argIndex) {
        ...
        throw new IllegalArgumentException("Ungültiger Argumentindex für JMP: " + argIndex);
    }
    ...
    public void execute(Simulation simulation) {
        ...
        if (!(deltaObj instanceof int[] v)) {
            organism.instructionFailed("JMP: Ungültiger Registertyp für Delta (Reg " + reg + "). Erwartet Vektor (int[]), gefunden: " + (deltaObj != null ? deltaObj.getClass().getSimpleName() : "null") + ".");
            return;
        }

        if (v.length != simulation.getWorld().getShape().length) {
            organism.instructionFailed("JMP: Dimension des Delta-Vektors stimmt nicht mit Welt-Dimension überein. Erwartet: " + simulation.getWorld().getShape().length + ", gefunden: " + v.length + ".");
            return;
        }
