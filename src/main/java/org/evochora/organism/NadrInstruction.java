// src/main/java/org/evochora/organism/NadrInstruction.java
...
public class NadrInstruction extends Instruction {
    public static final int ID = 5;

    private final int reg1, reg2;

    public NadrInstruction(Organism o, int r1, int r2) {
        super(o);
        this.reg1 = r1;
        this.reg2 = r2;
    }
    ...
    static {
        Instruction.registerInstruction(NadrInstruction.class, ID, "NADR", 3, NadrInstruction::plan, NadrInstruction::assemble);
    ...
    public ArgumentType getArgumentType(int argIndex) {
        ...
        throw new IllegalArgumentException("Ungültiger Argumentindex für NADR: " + argIndex);
    }
    ...
    public static Instruction plan(Organism organism, World world) {
        ...
        return new NadrInstruction(organism, r1, r2);
    }

    public static AssemblerOutput assemble(String[] args, Map<String, Integer> registerMap, Map<String, Integer> labelMap) {
        if (args.length != 2) {
            throw new IllegalArgumentException("NADR erwartet 2 Argumente: %REG1 %REG2");
        }
    ...
    public void execute(Simulation simulation) {
        ...
        if (val1Obj instanceof int[] || val2Obj instanceof int[]) {
            organism.instructionFailed("NADR: Vektoren sind nicht für bitweise Operationen erlaubt. Register (" + reg1 + ", " + reg2 + ") müssen skalare Integer sein.");
            return;
        }

        if (!(val1Obj instanceof Integer v1Raw) || !(val2Obj instanceof Integer v2Raw)) {
            organism.instructionFailed("NADR: Ungültige Registertypen. Beide Register (" + reg1 + ", " + reg2 + ") müssen skalare Integer sein.");
            return;
        }
        ...
        if (Config.STRICT_TYPING) {
            ...
            if (s1.type() != s2.type()) {
                organism.instructionFailed("NADR: Registertypen müssen übereinstimmen im strikten Modus. Reg " + reg1 + " (" + s1.type() + ") vs Reg " + reg2 + " (" + s2.type() + ").");
                return;
            }
